  ...
  <scroll-view class="container" scroll-y>
    ...
    <view class="card">
      ...
      <view class="config-row">
        <text>发音人:</text>
        <picker :range="voices" range-key="label" :value="voiceIndex" @change="onVoiceChange">
          <view class="picker-view">{{ voices[voiceIndex].label }}</view>
        </picker>
      </view>
      ...
      <view class="task-list">
        <view class="task-item" v-for="task in tasks" :key="task.id" :class="{ playing: currentTaskId === task.id }">
          <view class="task-info" @click="loadTrack(task)">
            ...
            <text class="task-meta">{{ task.status === 'completed' ? '✅' : '⏳' }} {{ getVoiceLabel(task.voice) }} · {{ formatDate(task.created_at) }}</text>
          </view>
...
// 跨端适配: H5 用空字符串走 vite proxy，原生 App 用真实 IP
// #ifdef H5
const apiBaseUrl = ''
// #endif
// #ifndef H5
...
const voices = ref([
  { label: '晓晓 (女声 - 柔和自然)', value: 'zh-CN-XiaoxiaoNeural' },
  { label: '云希 (男声 - 故事朗读)', value: 'zh-CN-YunxiNeural' },
  { label: '云健 (男声 - 文艺抒情)', value: 'zh-CN-YunjianNeural' },
  { label: '云扬 (男声 - 专业新闻)', value: 'zh-CN-YunyangNeural' },
  { label: '晓伊 (女声 - 活泼俏皮)', value: 'zh-CN-XiaoyiNeural' },
  { label: '晓曼 (粤语女声)', value: 'zh-HK-HiuMaanNeural' },
  { label: '晓臻 (闽南普通女声)', value: 'zh-TW-HsiaoChenNeural' }
])
...
function getVoiceLabel(voiceId) {
  const v = voices.value.find(item => item.value === voiceId)
  return v ? v.label.split(' ')[0] : voiceId
}
...
function setupAudioListeners() {
  ...
  // 完全放开原生按键拦截，遵循系统的默认行为（类似 Web 端的 mediaSession 不拦截 next/prev）
  // 以兼容不同耳机（部分耳机双击为暂停，有些为切歌）
}

function loadTrack(task) {
  // 如果点击的是当前任务，直接切换播放/暂停状态
...
function chooseFile() {
  ...
  uni.chooseFile({
    ...
    type: 'all',
    success: (res) => {
      // res.tempFiles[0].path 实际上是一个 blob URL 或原生 File 引用
      uploadFile(res.tempFiles[0])
    },
    fail: (err) => { console.log('取消选择或失败', err) }
  })
  // #endif

  // #ifndef H5
  // 移动端 App 使用内置选择图片的 API（如果后续引入扩展原生插件，再支持 PDF/TXT）
  uni.chooseImage({
    count: 1,
    success: (res) => {
      uploadFile(res.tempFiles[0])
...
function uploadFile(fileObj) {
  ...
  // 开始独立轮询进度
  pollProgress(reqId, 'upload')
  
  // filePath 必须传，对于 H5 平台它可能需要适配
  uni.uploadFile({
    ...
    success: (uploadFileRes) => {
      ...
      try {
        ...
        if (data.cleaned_text) {
          inputText.value = data.cleaned_text
...
function startSynthesize() {
  ...
  // 转换速率为后端支持的字符串 (+0%, +50%, -20%)
  const percentage = Math.round((rate.value - 1) * 100)
  const rateStr = percentage >= 0 ? `+${percentage}%` : `${percentage}%`
  
  pollProgress(reqId, 'synthesize')
  ...
  uni.request({
    ...
    data: {
      ...
      filename: `文段朗读 ${new Date().toLocaleTimeString()}`,
      voice: voices.value[voiceIndex.value].value,
      rate: rateStr,
...
function stopPolling(reqId) {
  if (pollingTimers[reqId]) {
    clearInterval(pollingTimers[reqId])
    delete pollingTimers[reqId]
  }
}
...
function pollProgress(reqId, type) {
  stopPolling(reqId)
  pollingTimers[reqId] = setInterval(() => {
